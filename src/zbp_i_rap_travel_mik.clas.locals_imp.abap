...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  MODIFY ENTITIES OF zi_rap_travel_mik IN LOCAL MODE
      ENTITY travel
  ...
  ENDMETHOD.
   ...
   METHOD CalculateTravelID.
    ...
    READ ENTITIES OF zi_rap_travel_MIK IN LOCAL MODE
      ENTITY Travel
...
*      " Clear state messages that might exist
    ...
    DATA business_data TYPE TABLE OF ZZ_TRAVEL_AGENCY_ES55173B5845B. "rap_mikz_travel_agency_es5.
    IF  agencies IS NOT INITIAL.
      ...
      TRY.
         ...
         NEW zcl_ce_rap_agency_mik( )->get_agencies(
            EXPORTING
      ...
      IF travel-AgencyID IS INITIAL OR NOT line_exists( business_data[ agencyid = travel-AgencyID ] ).
        ...
        APPEND VALUE #( %tky        = travel-%tky
                        ...
                        %msg        = NEW zcm_rap_mik(
                                          ...
                                          textid   = zcm_rap_mik=>agency_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-CustomerID IS INITIAL OR NOT line_exists( customers_db[ customer_id = travel-CustomerID ] ).
        ...
        APPEND VALUE #(  %tky        = travel-%tky
                         ...
                         %msg        = NEW zcm_rap_MIK(
                                           severity   = if_abap_behv_message=>severity-error
                                           textid     = zcm_rap_MIK=>customer_unknown
    ...
    LOOP AT travels INTO DATA(travel).
      ...
      IF travel-EndDate < travel-BeginDate.
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_MIK(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_MIK=>date_interval
      ...
      ELSEIF travel-BeginDate < cl_abap_context_info=>get_system_date( ).
        ...
        APPEND VALUE #( %tky               = travel-%tky
                        ...
                        %msg               = NEW zcm_rap_MIK(
                                                 severity  = if_abap_behv_message=>severity-error
                                                 textid    = zcm_rap_MIK=>begin_date_before_system_date
                                                 begindate = travel-BeginDate )
...
*   For the Travel Status we need the before-image from the database. We perform this for active (is_draft=00) as well as for drafts (is_draft=01) as we can't distinguish between edit or new drafts
      ...
      IF is_update_requested = abap_true.
        ...
        IF has_before_image = abap_true.
          ...
          IF update_granted = abap_false.
            APPEND VALUE #( %tky        = travel-%tky
                            %msg        = NEW zcm_rap_MIK( severity = if_abap_behv_message=>severity-error
                                                            textid   = zcm_rap_MIK=>unauthorized )
                          ) TO reported-travel.
          ENDIF.
  ...
  METHOD is_update_granted.
    IF has_before_image = abap_true.
      AUTHORITY-CHECK OBJECT 'ZOSTATMIK'
        ID 'ZOSTATMIK' FIELD overall_status
        ID 'ACTVT' FIELD '02'.
    ELSE.
      AUTHORITY-CHECK OBJECT 'ZOSTATMIK'
        ID 'ZOSTATMIK' DUMMY
